    ...
    <div>
      ...
      <div className="flex border-b mb-6">
        <button 
          ...
          className={`px-6 py-3 font-medium text-sm flex items-center gap-2 border-b-2 transition-colors ${activeTab === 'prayers' ? 'border-brand-600 text-brand-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
        >
        ...
        <button 
          ...
          className={`px-6 py-3 font-medium text-sm flex items-center gap-2 border-b-2 transition-colors ${activeTab === 'messages' ? 'border-brand-600 text-brand-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
        >
      ...
      {activeTab === 'prayers' && (
          <div className="space-y-4">
              {prayers.length === 0 && <p className="text-gray-500 text-center py-8">No prayer requests.</p>}
              {prayers.map(req => (
                  <Card key={req.id} className="p-5 border-l-4 border-l-brand-500">
                      <div className="flex justify-between items-start mb-2">
                          <div>
                              <h3 className="font-bold text-gray-900">{req.isAnonymous ? 'Anonymous' : req.requesterName}</h3>
                      ...
                      <p className="text-gray-800 my-3 bg-gray-50 p-3 rounded-lg">{req.content}</p>
      ...
      {activeTab === 'messages' && (
          <div className="space-y-4">
              {messages.length === 0 && <p className="text-gray-500 text-center py-8">No messages.</p>}
              {messages.map(msg => (
                  <Card key={msg.id} className={`p-5 ${!msg.isRead ? 'bg-blue-50/50 border-blue-200' : ''}`}>
                      <div className="flex justify-between items-start mb-2">
                          <div>
                              <h3 className={`font-bold text-gray-900 ${!msg.isRead ? 'text-blue-900' : ''}`}>{msg.senderName}</h3>
                              <p className="text-xs text-gray-500">{msg.phone}</p>
                          </div>
                          {!msg.isRead && <Badge color="red">New</Badge>}
                      </div>
                      <p className="text-gray-800 my-3">{msg.text}</p>
                      <div className="flex justify-between items-center mt-4">
                          <span className="text-xs text-gray-400">{new Date(msg.date).toLocaleString()}</span>
                          {!msg.isRead ? (
                              <Button size="sm" variant="ghost" onClick={() => handleMarkRead(msg.id)} className="text-blue-600 gap-1">
