...
const Layout: React.FC = () => {
  ...
  const MobileBottomNav = () => (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-nav-red border-t border-nav-red pb-[env(safe-area-inset-bottom)] z-50 flex justify-between items-center px-1 h-auto shadow-[0_-4px_10px_-1px_rgba(0,0,0,0.3)]">
      {NAV_ITEMS.filter(i => i.showInBottom).map((item) => (
        ...
        >
          {({ isActive }) => (
            <>
              <item.icon className={`w-5 h-5 sm:w-6 sm:h-6 transition-all ${isActive ? 'scale-110 drop-shadow-sm' : ''}`} />
              <span className={`text-[9px] sm:text-[10px] font-medium tracking-tight line-clamp-1 ${isActive ? 'font-bold' : ''}`}>{item.label}</span>
            </>
          )}
        </NavLink>
      ))}
      <button 
        ...
        className={`flex flex-col items-center justify-center flex-1 py-2 px-1 transition-colors ${mobileMenuOpen ? 'text-white' : 'text-white/50'}`}
      >
  ...
  const MobileMenuOverlay = () => {
    ...
    return (
      <div className="md:hidden fixed inset-0 z-[60] bg-app-bg animate-in slide-in-from-bottom-10 fade-in duration-200 flex flex-col overflow-hidden pt-[env(safe-area-inset-top)]">
        {/* Header */}
        <div className="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between border-b border-white/10 bg-slate-900/95 backdrop-blur-md flex-shrink-0">
        ...
        {/* User Card & Menu Grid */}
        <div className="p-4 sm:p-6 overflow-y-auto bg-gradient-to-b from-slate-900 to-app-bg flex-1">
           {user ? (
            <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-brand-600 rounded-xl sm:rounded-2xl text-white shadow-lg mb-6 border border-brand-500/50">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/20 flex items-center justify-center font-bold text-sm sm:text-lg border border-white/20 flex-shrink-0">
                    {user.name.charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-sm sm:text-lg leading-tight truncate">{user.name}</h3>
                    <p className="text-brand-100 text-[10px] sm:text-xs uppercase font-semibold tracking-wide truncate">{user.role.replace('_', ' ')}</p>
                </div>
                <button onClick={handleLogout} className="p-2 bg-black/20 rounded-full hover:bg-black/30 transition-colors flex-shrink-0">
                    <LogOut className="w-4 h-4 sm:w-5 sm:h-5" />
                </button>
            </div>
           ) : (
            <div className="mb-6">
                <Button onClick={() => navigate('/login')} className="w-full text-sm">Sign In</Button>
            </div>
           )}
           ...
           <div className="grid grid-cols-2 gap-2 sm:gap-3">
             {NAV_ITEMS.filter(i => !i.showInBottom).map(item => (
                <NavLink 
                  ...
                  to={item.to}
                  onClick={toggleMobileMenu}
                  className="flex flex-col items-center justify-center p-3 sm:p-4 bg-white rounded-lg sm:rounded-xl shadow-md active:scale-95 transition-all text-gray-900 gap-2"
                >
                   <div className="p-2 sm:p-3 bg-brand-50 rounded-full text-brand-700">
                      <item.icon className="w-5 h-5 sm:w-6 sm:h-6" />
                   </div>
                   <span className="font-medium text-xs sm:text-sm text-center line-clamp-2">{item.label}</span>
                </NavLink>
             ))}
             ...
             {user?.role === 'admin' && (
                <NavLink 
                  ...
                  className="flex flex-col items-center justify-center p-3 sm:p-4 bg-white rounded-lg sm:rounded-xl shadow-md active:scale-95 transition-all text-gray-900 gap-2"
                >
                   <div className="p-2 sm:p-3 bg-gray-100 rounded-full text-gray-700">
                      <Settings className="w-5 h-5 sm:w-6 sm:h-6" />
                   </div>
                   <span className="font-medium text-xs sm:text-sm text-center line-clamp-2">Settings</span>
                </NavLink>
             )}
           </div>
        </div>
  ...
  return (
    <div className="h-[100dvh] flex font-sans">
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 h-14 sm:h-16 bg-slate-900/40 backdrop-blur-md border-b border-white/10 flex items-center px-3 sm:px-4 justify-between z-40 pt-[env(safe-area-inset-top)]">
         ...
         {user ? (
           <NavLink to="/profile" className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-brand-600 border border-brand-400 text-white flex items-center justify-center font-bold text-xs sm:text-sm flex-shrink-0">
            ...
            <NavLink to="/login" className="text-[10px] sm:text-xs font-bold text-white bg-white/20 px-2 sm:px-3 py-1 rounded-full hover:bg-white/30 transition-colors flex-shrink-0">Login</NavLink>
         )}
      </header>
      ...
      <main className="flex-1 overflow-y-auto h-full w-full md:ml-72">
         ...
         <div className="pt-16 sm:pt-20 pb-20 sm:pb-24 md:pt-8 md:pb-8 px-3 sm:px-4 md:px-8 max-w-5xl mx-auto min-h-screen">
            <Outlet />
         </div>
      </main>
